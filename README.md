...
Copy the IMIS folder (in the web application source folder) containing the source files and paste it in C:\inetpub\wwwroot
Similarly, copy the Web Services folder (from the Web Service source folder) and paste it in C:\inetpub\wwwroot
...
- Enter the physical path name: C:\inetpub\wwwroot\openIMIS (unless you have installed IIS somewhere else)
Select Port 80
...
#### Globalisation ####
Depending on the server's initial configuration, the date format may differ from the expected DD/mm/YYYY by IMIS
To force the date format, go to the IMIS site, then select .NET Globalisation
Under Culture, select English (United Kingdom) (en-GB) as a culture
...
- For example: <code><add name="NP_CENTRALConnectionString" connectionString="Data Source=TPH-L14005\SQLEXPRESS;Initial Catalog=NP_CENTRAL;User ID=sa;Password=●●●●●●●●●●●●●●●●●●●●" providerName="System.Data.SqlClient" /></code>
...
- For example: <code><add name="CHF_CENTRALConnectionString" connectionString="Data Source=TPH-L14005\SQLEXPRESS;Initial Catalog=NP_CENTRAL;User ID=sa;Password=●●●●●●●●●●●●●●●●●●●●" providerName="System.Data.SqlClient" /></code>
